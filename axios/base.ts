...
base.interceptors.request.use(async config => {
    ...
    if (config.url == "/auth/register" || config.url == "/auth/refresh" ||  config.url == "/auth/login") {
        return config
    }
    return {
        ...config, headers: {
...
base.interceptors.response.use(
    ...
    async error => {
        ...
        if (error.code == "ERR_NETWORK") return Promise.reject(Errors.CONNECTION_ERROR)
        if (!error.response) return Promise.reject(Errors.UNEXPECTED_ERROR)
        switch (error.response.status) {
            case 401:
                if (error.config.url == "/auth/login") return Promise.reject(Errors.USER_NOT_FOUND)
